...
static int xkb_get_flag_size(XkbPlugin *p_xkb)
{
    int  size = plugin_get_icon_size(p_xkb->p_plugin);
...
static int xkb_constructor(Plugin * p_plugin, char ** fp)
{
    ...
    plugin_set_priv(p_plugin, p_xkb);
    ...
    plugin_set_widget(p_plugin, gtk_event_box_new());
    gtk_widget_add_events(plugin_widget(p_plugin), GDK_BUTTON_PRESS_MASK);

    /* Create a horizontal box as the child of the button. */
    GtkWidget * hbox = gtk_hbox_new(FALSE, 0);
    gtk_container_set_border_width(GTK_CONTAINER(hbox), 3);
    gtk_container_add(GTK_CONTAINER(plugin_widget(p_xkb->p_plugin)), hbox);
    ...
    /* Connect signals. */
    g_signal_connect(plugin_widget(p_plugin), "button-press-event", G_CALLBACK(on_xkb_button_press_event), p_xkb);
    g_signal_connect(plugin_widget(p_plugin), "scroll-event", G_CALLBACK(on_xkb_button_scroll_event), p_xkb);
    g_signal_connect(G_OBJECT(fbev), "active_window", G_CALLBACK(on_xkb_fbev_active_window_event), p_xkb);

    /* Show the widget and return. */
    xkb_redraw(p_xkb);
    gtk_widget_show(plugin_widget(p_plugin));
    return 1;
}
...
static void on_dialog_config_response(GtkDialog *p_dialog, gint response, gpointer data)
{
    ...
    panel_config_save(plugin_panel(p_xkb->p_plugin));
    xkb_redraw(p_xkb);
