...
#define SU_JSON_OPTION_STRUCTURE XkbPlugin
...
static int xkb_constructor(Plugin * plugin);
...
static void xkb_save_configuration(Plugin * p);
...
void xkb_redraw(XkbPlugin * xkb) 
{
    ...
    /* Set the label. */
    if ((xkb->display_as_text) || ( ! valid_image))
    {
        char * group_name = (char *) xkb_get_current_symbol_name(xkb);
        if (group_name != NULL)
        {
            panel_draw_label_text(plugin_panel(xkb->plugin), xkb->label, (char *) group_name, STYLE_BOLD | STYLE_CUSTOM_COLOR);
...
static int xkb_constructor(Plugin * plugin)
{
    ...
    xkb->display_as_text = FALSE;
    xkb->per_window_layout = TRUE;
    xkb->default_group = 0;

    su_json_read_options(plugin_inner_json(plugin), option_definitions, xkb);
...
static void xkb_configure(Plugin * p, GtkWindow * parent)
{
    ...
    gtk_combo_box_set_active(GTK_COMBO_BOX(display_type_optmenu), xkb->display_as_text);
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(per_app_checkbutton), xkb->per_window_layout);
...
{
    XkbPlugin * xkb = PRIV(p);
    su_json_write_options(plugin_inner_json(p), option_definitions, xkb);
}
